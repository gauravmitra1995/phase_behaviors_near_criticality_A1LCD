...
T_strings = ["50","52","54","55","56","57","58","58.50","58.75","59","59.25","59.50","59.75","60","61","62","64"]

Nchains   = 10000
Lsim      = 240
Nsubboxes = 10000
variant   = variant  # defined upstream
...
for k, L_box in enumerate(sorted(interps.keys())):
    ...
    plt.errorbar(np.array(d["T"])*5.6, d["U"], yerr=d["E"], marker='o', ms=8, capsize=10,
                 color=color, lw=1.8, elinewidth=1.3, label=r'$L = $'+f"{L_box}")

plt.ylabel(r'$U_{L}(T) = 1 - \frac{\langle \Delta \phi_L^4 \rangle}{3\langle \Delta \phi_L^2 \rangle^2}$')
plt.xlabel(r'$T\;(\mathrm{K})$')
plt.legend(loc='lower left', ncol=2)
plt.xticks(np.array([50,52,54,56,58,60,62,64])*5.6)
plt.tight_layout()
plt.savefig(f'bindercumulant_pairwisecrossing_plot.pdf', bbox_inches='tight')
