...
class SystemSetup(object):
    ...
    # TODO:
    # Add a function that checks if the ortho boxes are such that there are no conditions with _bead_ density
    # higher than 1.0.
...
class SimulationSetup(object):
    ...
    def _get_nested_dirs_for(self, sys_name="SysA", wInt: bool=True):
        """
    ...
    def create_dirs_for(self, sys_name="SysA", wInt: bool=True):
        """
    ...
    def write_params_for(self, sys_name, wInt: bool=True, fullRand: bool = True, cls_mode: int = 0):
        """
    ...
    def clean_dirs_for(self, sys_name="SysA", b_txt: bool=True, b_trajectory=True, b_data=True, wInt=True):
        """
    ...
    def _read_raw_data_files_for(self, sys_name, wInt: bool=True, file_name='__CLUS.dat'):
        """
        ...
        dum_nest_list = self._get_nested_dirs_for(sys_name=sys_name, wInt=wInt)
...
class _DataWithError(object):
    ...
    def calc_distribution_moment_k(self, moment: int = 1, normalized: bool = True):
        """
        ...
        :param moment: Which moment to calculate
        :return: moment_val, moment_err
        """
        
        assert moment > 0, "Moments should be positive!"
    ...
    def calc_distribution_mean(self, normalized: bool = True):
        """
    ...
    def calc_distribution_var(self):
        """
        ...
        first_moment_m, first_moment_e = self.calc_distribution_moment_k(k=1)
        first_moment = _DataWithError(np.array([1]), np.array([first_moment_m]), np.array([first_moment_e]))

        second_moment_m, second_moment_e = self.calc_distribution_moment_k(k=2)
